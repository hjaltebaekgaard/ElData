<?php

use Illuminate\Support\Facades\Http;
use Illuminate\Support\Facades\Route;

use App\Http\Controllers\SheetsApiController;
use App\Http\Controllers\DataPointController;

use Google\Client as Google_Client;
use Google\Service\Sheets;

/*
if (!isset($_GET['code'])) {
    $authUrl = $client->createAuthUrl();
    printf("Open the following link in your browser:\n%s\n", $authUrl);
    print 'Enter verification code: ';
    $authCode = trim(fgets(STDIN));

    // Exchange authorization code for an access token
    $accessToken = $client->fetchAccessTokenWithAuthCode($authCode);
    $client->setAccessToken($accessToken);

    // Save the access token to a file or database for future use
    file_put_contents(base_path() . 'access_token.json', json_encode($accessToken));
} else {
    // Load previously authorized credentials from a file or database
    $accessToken = json_decode(file_get_contents(base_path() . 'access_token.json'), true);
    $client->setAccessToken($accessToken);
} */

/* $client = NULL;
$service = NULL;
$psw = "";
$counter = 0; */

/*
|--------------------------------------------------------------------------
| Web Routes
|--------------------------------------------------------------------------
|
| Here is where you can register web routes for your application. These
| routes are loaded by the RouteServiceProvider and all of them will
| be assigned to the "web" middleware group. Make something great!
|
*/


$dataAccessToken = "BEARER *data_access_token*";
$refreshToken = "BEARER *refresh_token";


Route::get('/', function () use ($refreshToken, $client, $service, $psw) {
    dd($client, $service, $psw);
    $start = date('Y-m-d', now()->subDays(2)->timestamp);
    $end = date('Y-m-d', now()->subDay()->timestamp);
    return [$start, $end];
    // return view('welcome');
});

/* response from /token request {"result":"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..BrYwWrl-DSGHE0vPiVV3KobYT0KdZYHD2hDMLls1udc"} */

Route::get('/data/{meteringpointid}', function ($id) use ($dataAccessToken) {
    $start = date('Y-m-d', now()->subDays(2)->timestamp);
    $end = date('Y-m-d', now()->subDay()->timestamp);
    // Make a GET request to the REST API endpoint
    $url = "https://api.eloverblik.dk/customerapi/api/meterdata/gettimeseries/{$start}/{$end}/Hour";
    // dd($url);
    $headers = [
        "Content-Type" => 'application/json',
        "Authorization" => "BEARER {$dataAccessToken}",
        "accept" => 'application/json'
    ];

    // $headers->accept;
    $postData =
        [
            'meteringPoints' => [
                'meteringPoint' => [
                    "$id"
                ]
            ]
        ];
    // dd([$id, $headers, $postData]);
    $response = Http::withHeaders($headers)->post($url, $postData);

    // Check if request was successful
    if ($response->successful()) {
        // Get JSON data from the response

        $data = $response->json();
        return $data['result'][0]['MyEnergyData_MarketDocument']['TimeSeries'][0]['Period'];
    } else {
        // Handle error
        // dd($response);
        return response()->json(['error' => 'Failed to fetch data from API'], $response->status());
    }
});

Route::get('/getmeteringpoints', function () use ($dataAccessToken) {

    // Make a GET request to the REST API endpoint
    $headers = [
        "Authorization" => "BEARER {$dataAccessToken}",
        "accept" => 'application/json'
    ];

    $response = Http::withHeaders($headers)->get('https://api.eloverblik.dk/customerapi/api/meteringpoints/meteringpoints');

    // Check if request was successful
    if ($response->successful()) {
        // Get JSON data from the response

        $data = $response->json();
        // $test = $data["result"][0]["streetCode"];
        // dd($test);
        return $data;
    } else {
        // Handle error
        return response()->json(['error' => 'Failed to fetch data from API'], $response->status());
    }
});


Route::get('/gettoken', function () use ($refreshToken) {
    // Make a GET request to the REST API endpoint
    $headers = [
        "Authorization" => $refreshToken,
        "accept" => 'application/json'
    ];

    $response = Http::withHeaders($headers)->get('https://api.eloverblik.dk/customerapi/api/token');

    // Check if request was successful
    if ($response->successful()) {
        // Get JSON data from the response

        $data = $response->json();
        return $data;
    } else {
        // Handle error
        return response()->json(['error' => 'Failed to fetch data from API'], $response->status());
    }
});
/*
Route::get('/client', function () use ($client, $service, $psw) {
    var_dump(!$client);
    if (!$client) {
        $client = new Google_Client();
        $client->setApplicationName('ElDataDesktop-1');
        $client->setScopes(Sheets::SPREADSHEETS_READONLY);
        $client->setAuthConfig(base_path() . "/credentials.json");
        $client->setAccessType('offline');
        var_dump($client, !$client);
    } else {
        var_dump($client);
    }
}); */

// Route::get('/test', [SheetsApiController::class, 'testFunction']);
Route::get('/test', [DataPointController::class, 'testFunction']);
// Route::get('/test', ['DataPointController::class@testFunction']);

//non functional, figure out how to use it as a service
Route::get('/google', function () use ($client, $service, $psw) {

    if (!$client) {
        $client = new Google_Client();
        $client->setApplicationName('ElDataDesktop-1');
        $client->setScopes(Sheets::SPREADSHEETS_READONLY);
        $client->setAuthConfig(base_path() . "/credentials.json");
        $client->setAccessType('offline');
        $GLOBALS['client'] = $client;
    } else {
        var_dump($client);
    }
    // dd($client);
    // If not authenticated or accessToken is expired -> google authenticate
    $accessToken = json_decode(file_get_contents(base_path() . 'access_token.json'), true);
    dd($accessToken);
    if (!$accessToken || $client->isAccessTokenExpired()) {
        // Set the authorized redirect URI. This should match the one set in the OAuth credentials.
        $client->setRedirectUri("http://127.0.0.1:8000/google");
        $accessToken = $client->fetchAccessTokenWithAuthCode($psw);
        file_put_contents(base_path() . 'access_token.json', json_encode($accessToken));
    }

    $client->setAccessToken($accessToken);

    // If no service -> create service
    if (!$service) {
        $service = new Sheets($client);
    }

    // Send api request

    // var_dump response


    // $client->setDeveloperKey("AIzaSyDCEcreatqJiO8bP5CgP-bJGD8LQlQQwhs");

    $spreadsheetId = '1uHk2hL2MgDKjUhNIpo2G99RS9ORxEye4tjV1ZMAiuXc';
    $range = 'Sheet3!A1';
    $response = $service->spreadsheets_values->get($spreadsheetId, $range);
    $values = $response->getValues();

    if (empty($values)) {
        print "No data found.\n";
    } else {

        var_dump($response);
    }
});
